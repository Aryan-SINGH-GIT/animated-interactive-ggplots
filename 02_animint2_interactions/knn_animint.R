...
knn_animint <- function(train, test, class.vec, k = 30, 
  ...
  train.df$train.id <- 1:nrow(train.df)
  ...
  for (test.idx in 1:n.test) {
    ...
    train.sorted$rank <- 1:nrow(train.sorted)
    ...
    for (k.val in k.sequence) {
      ...
      # Title
      title.text <- sprintf("Test Point %d/%d | k = %d | Predicted: %s", 
  ...
  p.feature <- ggplot() +
    geom_polygon(data=polygon.all, aes(x=x, y=y, group=poly.group), fill="#FFFFCC", alpha=0.4, showSelected="frame.id") +
    geom_path(data=circle.all, aes(x=x, y=y, group=circle.group), color="gray40", linetype="dashed", size=1, showSelected="frame.id") +
    geom_segment(data=lines.all, aes(x=x.start, y=y.start, xend=x.end, yend=y.end, group=line.group), color="gray60", linetype="dotted", size=0.8, alpha=0.6, showSelected="frame.id") +
    geom_point(data=training.all, aes(x=x, y=y, fill=class, alpha=point.alpha, stroke=I(point.stroke)), shape=21, color="gray50", size=2.5, showSelected="frame.id") +
    scale_alpha_identity() +
    geom_point(data=test.prev.all, aes(x=x, y=y, fill=class), shape=21, color="black", size=2.5, alpha=1.0, stroke=1.5, showSelected="frame.id") +
    geom_text(data=test.future.all, aes(x=x, y=y), label="?", color="#8B4513", size=8, fontface="bold", vjust=0, showSelected="frame.id") +
    geom_point(data=test.current.all, aes(x=x, y=y), shape=21, fill="#FF0000", color="black", size=3, alpha=1.0, showSelected="frame.id") +
    geom_point(data=test.current.all, aes(x=x, y=y), shape=21, fill="transparent", color="#FF0000", size=4, stroke=2, showSelected="frame.id") +
    geom_text(data=title.all, aes(x=x, y=y, label=label), size=5, fontface="bold", vjust=0, showSelected="frame.id") +
  ...
  # Vote Chart
  p.votes <- ggplot() +
    geom_bar(data=votes.all, aes(x=class, y=votes, fill=class), stat="identity", position="identity", width=0.6, showSelected="frame.id") +
    scale_fill_manual(values=class.colors) +
    scale_y_continuous(breaks=seq(0, k, by=5), limits=c(0, k)) +
    labs(title="Classification Votes", x="Class", y="Votes") +
    theme_bw() +
    theme(
  ...
  animint(
    ...
    time = list(variable="frame.id", ms=interval),
...
viz <- knn_animint(
  train = x,
  test = y,
  class.vec = rep(c("first class", "second class"), each = 40),
  k = 30,
  interval = 1000
)
...
animint2pages(
  plot.list = viz, 
  github_repo = "knn-animint", 
  owner = "Aryan-SINGH-GIT", 
  commit_message = "Update KNN animation with equal sized plots"
)